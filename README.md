...
fresh installation. Analytics start disabled, and a fresh install asks whether
to enable [minimal anonymous usage analytics](docs/analytics.md), with Yes as the
default. It then offers optional backup setup; a maintenance-only installation
does not require Homebrew or GitHub CLI. When configured, backups can run
automatically with `ballin update`.
