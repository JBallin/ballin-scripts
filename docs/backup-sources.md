...
| `~/.ballin-scripts/ballin.config.json` | `ballin_config` | Recover supported Ballin preferences. | Five maintenance choices. The backup destination, local analytics setting and installation identity, automatic-backup setting, sensitive-source consent, and custom settings are excluded. | Preferences; filtered export. |
