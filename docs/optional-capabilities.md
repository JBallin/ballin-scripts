...
excluded, default-included inventories can reveal identities, private or custom
URLs, and paths; Ballin does not scan or redact credentials. GitHub controls
account-based commit author/committer attribution; Ballin changes no global Git
identity configuration. GitHub retains history, but Ballin provides no history
navigation, rollback, or revision picker.
